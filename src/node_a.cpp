...
String my_ip_message = String("NODE A - IP: ");

AES256 aes;

char key[] = "sua-chave-secreta";
...
void processReceivedPackets(void *)
{
  for (;;)
  {
    ...
    while (radio.getReceivedQueueSize() > 0)
    {
      ...
      byte decrypted[16];
      aes.decryptBlock((uint8_t*) decrypted, (uint8_t*) packet->payload->message);
      Serial.println("Decrypted:");
      Serial.println((char *) decrypted);

      last_message = String((char *)decrypted);
...
{
  display.clearBuffer();
  String message_to_draw = String("Last message: " + last_message);
  display.drawStr(0, 10, my_ip_message.c_str());
  display.drawStr(0, 20, message_to_draw.c_str());
